...
public class MyBarChart {
    public void setGraphChart(Context context, BarChart chart, List<Report> reports,int basicBurned){
        ...
        int minday = 0;
        int maxday = 31;
        Comparator<Report> Comparator = (a, b) -> a.getReportdate().compareTo(b.getReportdate());
        List<Report> sortedReport = reports.stream().sorted(Comparator).collect(Collectors.toList());
        for (Report r: sortedReport)
        {
            int date = Integer.parseInt(new SimpleDateFormat("dd").format(r.getReportdate()));
